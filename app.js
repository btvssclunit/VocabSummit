...
(function () {
  ...
  function miniHorizon() {
    return '<div class="mini-horizon horizon">' +
      '<img class="mh-img" src="hero_bg.png" alt="">' +
  ...
  /* ==================================================================
     ...
     ================================================================== */
  /* progress-mapped slice of the vertical panorama: p=0 summit … p=1 base */
  function drawPanorama(ctx, W, H, p) {
    if (!SPRINT_BG.complete || !SPRINT_BG.naturalWidth) return false;
    var iw = SPRINT_BG.naturalWidth, ih = SPRINT_BG.naturalHeight;
    var sh = ih * 0.45, sw = sh * W / H;
    if (sw > iw) { sw = iw; sh = sw * H / W; }
    var sy = Math.max(0, Math.min(ih - sh, p * (ih - sh)));
    var sx = (iw - sw) / 2;
    ctx.drawImage(SPRINT_BG, sx, sy, sw, sh, 0, 0, W, H);
    ctx.fillStyle = "rgba(18,36,66,.16)";           // readability scrim
  ...
  function startSprint() {
    ...
    function frame(t) {
      ...
      var pano = drawPanorama(ctx, W, H, camY / Math.max(1, worldH() - H));
      if (!pano) {
      ...
      if (!pano) {
        ...
        /* drifting pixel clouds (slow parallax, wrap vertically) */
        var per = H + 200;
      ...
      ctx.lineCap = "round";
      ctx.strokeStyle = "#5E7A45"; ctx.lineWidth = 52; tracePath();   // grassy rim
      ctx.strokeStyle = "#8A6B43"; ctx.lineWidth = 42; tracePath();   // earth
      ctx.strokeStyle = "#C89B5A"; ctx.lineWidth = 30; tracePath();   // sunlit dirt
      /* landmarks beside the trail (deterministic per altitude) */
      for (var la = a0; la <= a1; la++) {
        var side = (la % 2 === 0) ? -1 : 1;
        var ly2 = yOf(la) - camY + 8;
        if (la % 7 === 3) drawTile("pine", xOf(la) + side * 78, ly2, la % 14 === 3 ? 1 : 0.8);
        if (la % 21 === 8) drawTile("sign", xOf(la) + side * 62, ly2 - 2, 1);
        if (la % 42 === 30) {
          drawTile("tent", xOf(la) + side * 84, ly2, 1);
          drawTile("fire", xOf(la) + side * 84 + (side < 0 ? 46 : -46), ly2, 0.8);
        }
      }
      /* summit pavilion + flag beside the sun */
      if (totalAlt >= a0 && totalAlt <= a1) {
        drawTile("pavilion", xOf(totalAlt), yOf(totalAlt) - camY - 12, 1);
        drawTile("flag", xOf(totalAlt) - 64, yOf(totalAlt) - camY - 16, 1);
      }
      /* stone steps on the trail */
      for (var s = a0; s <= a1; s++) {
        drawTile("steps", xOf(s), yOf(s) - camY + 11, 0.82);
      }
      ...
      if (best > 0 && startAlt + best <= totalAlt) {
        ...
        if (ly > -20 && ly < H + 20) {
          ...
          drawTile("flag", 24, ly + 2, 0.62);
          ctx.font = "13px sans-serif"; ctx.fillStyle = "#FFE9BD";
          ctx.fillText("个人纪录", 42, ly - 6);
        }
      }
