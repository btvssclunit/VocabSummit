...
(function () {
  ...
  /* ==================================================================
     ...
     ================================================================== */
  /* 11 waypoints expressed as fractions of canvas [x, y] — base(0) → summit(1.0) */
  var SPRINT_WP = [
    [0.50, 0.93], // 0  lake base
    [0.38, 0.83], // 1
    [0.62, 0.73], // 2
    [0.35, 0.63], // 3
    [0.65, 0.54], // 4
    ...
    [0.55, 0.22], // 8
    [0.45, 0.14], // 9
    [0.50, 0.07]  // 10 summit peak
  ];
  /* interpolate position along the waypoint path; t = 0 (base) → 1 (summit) */
  function waypointPos(t, W, H) {
    var n = SPRINT_WP.length - 1;
    var seg = Math.min(n - 0.001, t * n);
    var i = Math.floor(seg), f = seg - i;
    var a = SPRINT_WP[i], b = SPRINT_WP[Math.min(n, i + 1)];
    return { x: (a[0] + (b[0] - a[0]) * f) * W, y: (a[1] + (b[1] - a[1]) * f) * H };
  }
  ...
  function startSprint() {
    ...
    /* fraction of mountain climbed, clamped 0-1 */
    function climbFrac(alt) { return Math.max(0, Math.min(1, alt / Math.max(1, totalAlt))); }
    ...
    function frame(t) {
      ...
      var frac = climbFrac(climbAlt);
      var pos = waypointPos(frac, W, H);
      var slipX = slipT > 0 ? Math.sin(slipT * 25) * 3.5 : 0;
      var px = pos.x + slipX, py = pos.y;

      /* sprite facing: tangent from slightly ahead vs behind */
      var posA = waypointPos(Math.max(0, frac - 0.02), W, H);
      var posB = waypointPos(Math.min(1, frac + 0.02), W, H);
      var faceLeft = posB.x < posA.x;

      /* draw background */
      var pano = drawPanorama(ctx, W, H, frac);
      if (!pano) {
      ...
      /* glowing trail: path already climbed */
      ctx.lineCap = "round";
      var n = SPRINT_WP.length - 1;
      var climbSeg = frac * n;
      ctx.strokeStyle = "rgba(255,230,100,.55)"; ctx.lineWidth = 7;
      ctx.shadowColor = "rgba(255,210,60,.7)"; ctx.shadowBlur = 9;
      ctx.beginPath();
      for (var wi = 0; wi <= Math.ceil(climbSeg) && wi <= n; wi++) {
        var wpx = SPRINT_WP[wi][0] * W, wpy = SPRINT_WP[wi][1] * H;
        if (wi === 0) ctx.moveTo(wpx, wpy); else ctx.lineTo(wpx, wpy);
      }
      if (climbSeg < n) {
        var pEnd = waypointPos(frac, W, H);
      ...
      for (var wj = 0; wj <= n; wj++) {
        var wpx2 = SPRINT_WP[wj][0] * W, wpy2 = SPRINT_WP[wj][1] * H;
        if (wj === 0) ctx.moveTo(wpx2, wpy2); else ctx.lineTo(wpx2, wpy2);
      }
      ctx.stroke();

      /* personal-record marker on path */
      if (best > 0) {
        var bestFrac = climbFrac(startAlt + best);
        var bestPos = waypointPos(bestFrac, W, H);
        ctx.strokeStyle = "rgba(183,121,31,.85)"; ctx.lineWidth = 2; ctx.setLineDash([5, 4]);
        ctx.beginPath(); ctx.moveTo(bestPos.x - 28, bestPos.y); ctx.lineTo(bestPos.x + 28, bestPos.y); ctx.stroke();
        ctx.setLineDash([]);
        ctx.font = "bold 11px sans-serif"; ctx.fillStyle = "#FFE9BD"; ctx.textAlign = "center";
        ctx.fillText("个人纪录", bestPos.x, bestPos.y - 8);
        ctx.textAlign = "left";
      }
